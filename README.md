...
1. Скопировать файл PowersOfAttorney\SamplesOutput в каталог "Путь к установленному Web-клиент\Plugins"
2. Перезапустить IIS.
3. Добавить групповую опрерацию "Подпись доверенности" в разметку папки в кострукторе разметок.
4. Открыть папку с ПКД в представлении "МЧД единого формата - представление". редставлении должны присутствовать скрытые поля State и KindId. Выделить при помощи чекбоксов документы для подписания. На панели групповых операций нажать кнопку "Подписать доверенность".
