...
5. Откройте папку с ПКД в представлении "МЧД единого формата - представление". В представлении должны присутствовать поля State и KindId (допустимо, если они будут скрытые). Эти поля должны быть указаны в параметрах ЭУ "Подписать доверенности" в разметке DV.Folder_M4D.
...
Управлять видимостью кнопки "Отправить" возможно с помощью клиентского скрипта. Необходимо получить текущий статус регистрации МЧД с помощью метода getPowerOfAttorneyRegistrationStatus сервиса $EdiPowerOfAttorney (семпл веб-расширения "Модуль интеграции с операторами ЭДО").
В скрипте, в зависимости от полученного статуса, скрывайте/показывайте кнопку "Отправить".
Пример получания статуса регистарции можно найти в методе edi_getPowerOfAttorneyRegistrationStatus семпла веб-расширения "Модуль интеграции с операторами ЭДО".
7. Выделите при помощи чекбоксов документы для подписания. На панели групповых операций нажмите кнопку "Подписать доверенность".
